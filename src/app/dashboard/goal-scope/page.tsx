...
import {
  ...
  Beaker, Globe2, ArrowRight, ChevronDown, ChevronUp,
} from "lucide-react"
...
export default function GoalScopePage() {
  ...
  const isComplete = studyGoal.trim().length > 10 && functionalUnit.trim().length > 3
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-neutral-200 pb-5">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">
            Fondasi metodologi LCA. Wajib diisi sebelum modul lain dapat diproses. Mengunci parameter perhitungan seluruh modul berikutnya.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {isComplete && <span className="flex items-center gap-1.5 text-xs font-medium text-emerald-700"><CheckCircle2 className="h-4 w-4" /> Siap dikunci</span>}
          <Button onClick={handleSave} disabled={!isComplete}>
            {saved ? <><CheckCircle2 className="mr-2 h-4 w-4" />Tersimpan</> : <><Lock className="mr-2 h-4 w-4" />Simpan & Kunci Scope</>}
          </Button>
        </div>
      </div>
      ...
      {isComplete && (
        <div className="flex items-center justify-between rounded-xl bg-emerald-50 px-4 py-4 border border-emerald-200">
          <div>
            <p className="text-sm font-semibold text-emerald-900">Scope siap dikunci</p>
          ...
          <Button onClick={handleSave}>Kunci Sekarang <ArrowRight className="ml-2 h-4 w-4" /></Button>
        </div>
      )}
    </div>
  )
