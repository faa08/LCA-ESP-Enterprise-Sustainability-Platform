...
function KpiPanel({ kpis, onClose }: { kpis: CalculatedKPIs; onClose: () => void }) {
  const kpiRows = [
    ...
    { label: "PM (Particulate Matter)", value: fmt(kpis.pm_kgPM25e, 4), unit: "kg PM₂.₅e", color: "text-neutral-700" },
    ...
    <Section meta={meta} entryCount={entries.length}>
      {!showForm && (
        <Button size="sm" variant="secondary" onClick={() => setShowForm(true)} className="mb-4">
          <Plus className="h-3.5 w-3.5" /> Tambah Entri Produksi
        </Button>
      )}
      {showForm && (
...
function EnergyForm({ siteId, industryId, role, onCalcUpdate }: { siteId: string; industryId: string; role: string; onCalcUpdate: () => void }) {
  ...
  return (
    <Section meta={meta} entryCount={entries.length}>
      ...
      {!showForm && (
        <Button size="sm" variant="secondary" onClick={() => setShowForm(true)} className="mb-4">
          <Plus className="h-3.5 w-3.5" /> Tambah Data Energi
        </Button>
      )}
      {showForm && (
...
function LabForm({ siteId, industryId, role }: { siteId: string; industryId: string; role: string }) {
  ...
  return (
    <Section meta={meta} entryCount={entries.length}>
      ...
      {!showForm && <Button size="sm" variant="secondary" onClick={() => setShowForm(true)} className="mb-4"><Plus className="h-3.5 w-3.5" /> Tambah Hasil Lab</Button>}
      {showForm && (
...
function StackForm({ siteId, industryId, role, onCalcUpdate }: { siteId: string; industryId: string; role: string; onCalcUpdate: () => void }) {
  ...
  return (
    <Section meta={meta} entryCount={entries.length}>
      ...
      {!showForm && <Button size="sm" variant="secondary" onClick={() => setShowForm(true)} className="mb-4"><Plus className="h-3.5 w-3.5" /> Tambah Data Emisi Cerobong</Button>}
      {showForm && (
...
function B3Form({ siteId, industryId, role }: { siteId: string; industryId: string; role: string }) {
  ...
  return (
    <Section meta={meta} entryCount={entries.length}>
      ...
      {!showForm && <Button size="sm" variant="secondary" onClick={() => setShowForm(true)} className="mb-4"><Plus className="h-3.5 w-3.5" /> Tambah Data Limbah B3</Button>}
      {showForm && (
...
function TransportForm({ siteId, industryId, role, onCalcUpdate }: { siteId: string; industryId: string; role: string; onCalcUpdate: () => void }) {
  ...
  return (
    <Section meta={meta} entryCount={entries.length}>
      ...
      {!showForm && <Button size="sm" variant="secondary" onClick={() => setShowForm(true)} className="mb-4"><Plus className="h-3.5 w-3.5" /> Tambah Data Transportasi</Button>}
      {showForm && (
