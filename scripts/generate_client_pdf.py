...
class ExecutiveNumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        # Header
        self.drawString(54, 750, "ensPR — Executive Client Presentation & Deck")
        ...
        self.drawString(54, 36, "ensPR Enterprise Sustainability Platform — Executive Proposal & Deck")
        self.restoreState()
...
def parse_markdown_to_story(md_path, styles):
    ...
    for line in lines:
        ...
        if stripped.startswith('# '):
            ...
            story.append(Paragraph(clean_text, styles['DocList']))
...
def build_pdf():
    pdf_filename = "Buku_Panduan_dan_Proposal_Klien_ensPR.pdf"
    ...
    styles.add(ParagraphStyle('CoverSubtitle', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=13, leading=18, textColor=colors.HexColor("#059669"), alignment=1))
    styles.add(ParagraphStyle('CoverMeta', parent=styles['Normal'], fontName='Helvetica', fontSize=10, leading=14, textColor=colors.HexColor("#475569"), alignment=1))

    styles.add(ParagraphStyle('DocH1', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=17, leading=21, textColor=colors.HexColor("#0f172a")))
    styles.add(ParagraphStyle('DocH2', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=13, leading=17, textColor=colors.HexColor("#059669")))
    styles.add(ParagraphStyle('DocH3', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=11, leading=15, textColor=colors.HexColor("#1e293b")))
    styles.add(ParagraphStyle('DocBody', parent=styles['Normal'], fontName='Helvetica', fontSize=9.5, leading=14, textColor=colors.HexColor("#334155")))
    styles.add(ParagraphStyle('DocList', parent=styles['Normal'], fontName='Helvetica', fontSize=9.5, leading=14, textColor=colors.HexColor("#334155"), leftIndent=10))
    styles.add(ParagraphStyle('DocBullet', parent=styles['Normal'], fontName='Helvetica', fontSize=9.5, leading=14, textColor=colors.HexColor("#334155"), leftIndent=15))
    ...
    story.append(Paragraph("EXECUTIVE CLIENT PRESENTATION & PRODUCT DECK", styles['CoverSubtitle']))
    story.append(Spacer(1, 20))
    story.append(HRFlowable(width="60%", thickness=2, color=colors.HexColor("#059669"), spaceBefore=10, spaceAfter=20))
    story.append(Spacer(1, 120))
    story.append(Paragraph("<b>Target Audience:</b> Board of Directors, General Managers, EHS Directors, & Procurement Teams<br/><b>Core Pillars:</b> Life Cycle Assessment (LCA) + KLHK PROPER Compliance + Carbon Credit Monetization<br/><b>Website:</b> https://enspr.id", styles['CoverMeta']))
    story.append(PageBreak())

    md_path = "docs/Panduan_dan_Proposal_Klien_ensPR.md"
    doc_story = parse_markdown_to_story(md_path, styles)
    story.extend(doc_story)

    doc.build(story, canvasmaker=ExecutiveNumberedCanvas)
    print(f"SUCCESS: Generated 15-section executive client PDF '{pdf_filename}'!")
